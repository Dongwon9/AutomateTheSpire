...
public class EventScreenUtils {
    ...
    public static EventDialogType getEventDialogType() {
        boolean genericShown = ReflectionHacks.getPrivateStatic(
                GenericEventDialog.class, "show");
        if (genericShown) return EventDialogType.IMAGE;
        boolean roomShown = ReflectionHacks.getPrivate(
                (AbstractDungeon.getCurrRoom()).event.roomEventText,
                RoomEventDialog.class, "show");
        if (roomShown) return EventDialogType.ROOM;
    ...
    public static ArrayList<LargeDialogOptionButton> getActiveEventButtons() {
        ...
        for (LargeDialogOptionButton button : buttons) {
            if (!button.isDisabled) activeButtons.add(button);
        }
